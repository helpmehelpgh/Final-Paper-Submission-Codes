...
def preprocess_experiment_v3(prefix: str, base_dir: str, k: int = 5) -> pd.DataFrame:
    """
    Multi-signal ACC preprocessing with smaller history length to reduce overfitting.
...
def build_full_acc_dataframe_v3(base_dir: str, k: int = 5) -> pd.DataFrame:
    ...
    for prefix in prefixes:
        try:
            df = preprocess_experiment_v3(prefix=prefix, base_dir=base_dir, k=k)
            all_data.append(df)
        except FileNotFoundError:
            continue

    if not all_data:
        raise FileNotFoundError("No valid experiment groups were found for ACCDataset v3.")

    return pd.concat(all_data, ignore_index=True)


def split_by_experiment_v3(
...
def _get_feature_columns_v3(k: int = 5) -> List[str]:
    feature_cols = [
...
def prepare_acc_data_v3(
    ...
    full_df = build_full_acc_dataframe_v3(base_dir=base_dir, k=k)

    train_df, test_df, train_experiments, test_experiments = split_by_experiment_v3(
        full_df=full_df,
    ...
    feature_cols = _get_feature_columns_v3(k=k)
...
class ACCNetV3(nn.Module):
    """
    Smaller regularized MLP to reduce overfitting.
    """
...
class ACCTrainerV3:
    ...
    def save(self, file_name: str = "accnet_v3.onnx", in_features: int = 32) -> Path:
        self.model.eval()
    ...
    def plot_history(self, save_prefix: str = "results/acc_v3") -> None:
