...
from mchnpkg.deepl import prepare_acc_data_v2, ACCDatasetV2, ACCNetV2, ACCTrainerV2


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ACC state classifier v2 training script")
    parser.add_argument("--data_dir", type=str, default="/data/CPE_487-587/ACCDataset")
    parser.add_argument("--output_dir", type=str, default="results/acc_v2")
...
def main() -> None:
    ...
    print("Preparing ACC dataset v2...")
    data = prepare_acc_data_v2(
    ...
    train_dataset = ACCDatasetV2(X_train, y_train)
    test_dataset = ACCDatasetV2(X_test, y_test)
    ...
    model = ACCNetV2(in_features=X_train.shape[1])

    trainer = ACCTrainerV2(
        model=model,
    ...
    trainer.plot_history(save_prefix=str(output_dir / "acc_v2"))
    onnx_path = trainer.save(
        file_name=str(output_dir / "accnet_v2.onnx"),
